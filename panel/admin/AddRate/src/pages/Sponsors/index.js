...
import { BACKEND_URL_Micple_App, PROMOTIONS_BACKEND_URL } from "../../shared/constants/Variables";
...
const Headers = <Grid className="ads-header-sections" container>

  <Grid className="all-header" item xs={1} >Country</Grid>
  <Grid className="all-header" item xs={1} > Username</Grid>

  <Grid className="all-header" item xs={2} >Ads ID</Grid>

  {/* All details data  header sactions */}
  <Grid className="all-header" item xs={3} >Details</Grid>

  <Grid className="all-header" item xs={1}>Target People</Grid>

  <Grid className="all-header" item xs={2} >Transcation ID </Grid>


  <Grid className="all-header" item xs={1} >Notes</Grid>

  <Grid className="all-header" item xs={1}>Status</Grid>
...
function Sponsors({ match: { url }, history: { push } }) {
  ...
  useEffect(() => {
    let unmounted = false;
    if (!unmounted) {
      axios.get(`${PROMOTIONS_BACKEND_URL}/country?sort=country_name`,
        { headers: adminHeader() }
      ).then(({ data }) => {
  ...
  const getUserWiseAdIds = () => {
    ...
    axios.get(`${PROMOTIONS_BACKEND_URL}/add/filter-adds`,
  ...
  const getCountryWiseUsers = () => {
    ...
    axios.post(`${PROMOTIONS_BACKEND_URL}/add/getAdds`, body,
      { headers: adminHeader() }
    ).then(({ data }) => {
  ...
  const getAdidwisePaymentAndFilterAds = () => {
    ...
    axios.get(`${PROMOTIONS_BACKEND_URL}/payment/filter-payments`,
      { params: payment, headers: adminHeader() }
    ).then(({ data }) => {
  ...
  const searchHandler = () => {
    ...
    axios.get(`${PROMOTIONS_BACKEND_URL}/add/transaction-notes-wise-addId`,
      { params: searchFilter, headers: adminHeader() },
    ).then(({ data }) => {
  ...
  const rejectedAds = (id, postId) => {
    ...
    const promise1 = axios.patch(`${PROMOTIONS_BACKEND_URL}/add/change-report-status/${id}`, addStatusBody,
      { headers: adminHeader() }
    )


    const promise2 = axios.patch(`${BACKEND_URL_Micple_App}/promotion-app/change-status-admin/${postId}`,
      postStatusBody,
      { headers: adminHeader() })
  ...
  const approvedAds = (id, postId) => {
    ...
    const promise1 = axios.patch(`${BACKEND_URL_Micple_App}/promotion-app/change-status-admin/${postId}`,
      promoteStatus,
      { headers: adminHeader() }
    );

    const promise2 = axios.patch(`${PROMOTIONS_BACKEND_URL}/add/change-status/${id}`,
      addStatus,
      { headers: adminHeader() }
    );
  ...
  const bannedAdsAfterRunning = (id, postId) => {
    ...
    const promise1 = axios.patch(`${PROMOTIONS_BACKEND_URL}/add/ban-ads/${id}`, adsStatus,
      { headers: adminHeader() }
    )
  ...
  const deleteHandler = (id) => {
    ...
    const promise1 = axios.delete(`${PROMOTIONS_BACKEND_URL}/add/delet-ads-by-admin/${id}`, {
      headers: adminHeader()
    });
  ...
  return (
    ...
    >
      ...
      <AdsFilter onInputChange={onInputChange}
      ...
      <div className="adsData">
        ...
        <Grid container>
          ...
          {/* Country */}
          <Grid
            className="country-showign-section"
            item
            xs={1}
          >
          ...
          {selectedItem === null ? (
            ...
            <Grid
              className="username-showing-section"
              item
              xs={1}
            >
            ...
            >
              {
                ...
                adIds?.map((item, index) => (
                  ...
                  <div className="ad-id-all-items-sections" key={index}>
                    <p
                      onClick={() => setSelectedPost(item?.addId ? item?.addId : item?.add_id)}
                      className="ads-items"
                      style={{
                        backgroundColor:
                          selectedPost == (item?.addId || item?.add_id) ? "#e4e4e4" : "white",
                      }}
                    >
                          ...
                          backgroundColor:
                            item?.status === 0 ? "blue" :
                              item?.status === 1 ? "yellow" :
                                item?.status === 3 ? "#009cde" :
                                  item?.status === 10 ? "red" :
                                    item?.status === 11 ? "#8d2a2a" :
                                      item?.status === 5 ? "green" : "#0048ba"
                        }}
                      ></div>
                    </div>
          ...
          {selectedPost === null ? (
            ...
            <Grid
              ...
              className="ads-id-wise-details-sections"
              item
              xs={3}
            >
          ...
          {selectedPost === null ? (
            ...
            <Grid
              className="target-people-sections"
              item
              xs={1}
            >
          ...
          {selectedPost === null ? (
            ...
            <Grid
              className="transcation-id-sections"
              item
              xs={2}
            >

              <p
                className="transcation-id"
              >
              ...
              {selectedPostData.map((row, index) => (
                ...
                >

                  {
                    row?.status == 0 &&
                    <div>
                      <button className="btn-approve" onClick={() => approvedAds(row?._id, row?.postId)}>Approve</button>
                  ...
                  }
                  {
                    row?.status == 1 &&
                    <div>
                      <div style={{ color: "green" }}>Watiing For complete payment Procedure.</div>
                      <button className="btn-delete"
                        onClick={() => handleDeleteShow(row?._id)}
                      >Delete</button>
                    </div>
                  ...
                  {/*Payment already done need to verify it and run it as a add.*/}
                  {
                    ...
                    <div>
                      <button
                        onClick={() => completeAdsForRunning(row?._id, row?.postId)}
                        className="complete-ads-after-payment-done">Runs Ads</button>
                  ...
                  {/* Ads already running*/}
                  {
                    ...
                    <div className="runing-ads-sections">
                      ...
                      <button
                        onClick={() => bannedAdsAfterRunning(row?._id, row?.postId)}
                        className="ban-ads-after-payment-done">Ban</button>
...

                  {
                    row?.status == 10 &&
                    <div>
                      <div style={{ color: "red" }}>Ads is rejected For Bad content.</div>
                      <button className="btn-delete"
                        onClick={() => handleDeleteShow(row?._id)}
                      >Delete</button>
                    </div>
...

                  {
                    row?.status == 13 &&
                    <div>
                      <div style={{ color: "red" }}>Ads is Banned Parmanently.</div>
                      <button
                        onClick={() => handleDeleteShow(row?._id)}
